...
int vztt2_create_appcache(struct options_vztt *opts_vztt, int recreate)
{
	...
	tmpl_get_cache_tar_name(path, sizeof(path), tc.archive,
				get_cache_type(&gc, opts_vztt->image_format, opts_vztt->vefstype), opts_vztt->vefstype, gc.template_dir, os_app_name);
	...
	tmpl_get_cache_tar_name(base_cachename, sizeof(base_cachename),
		tc.archive, get_cache_type(&gc, opts_vztt->image_format, opts_vztt->vefstype), opts_vztt->vefstype,
		gc.template_dir,
		tmpl->os->name);
...
int vztt2_update_appcache(struct options_vztt *opts_vztt)
{
	...
	tmpl_get_cache_tar_name(path, sizeof(path), tc.archive,
				get_cache_type(&gc, opts_vztt->image_format, opts_vztt->vefstype), opts_vztt->vefstype,
				gc.template_dir, os_app_name);
...
int info_appcache(struct options_vztt *opts_vztt)
{
	...
	if (gc.veformat && tmpl_get_cache_tar_by_type(path, sizeof(path), get_cache_type(&gc, opts_vztt->image_format, opts_vztt->vefstype), opts_vztt->vefstype,
				gc.template_dir, os_app_name) != 0) {
		rc = VZT_TMPL_NOT_CACHED;
