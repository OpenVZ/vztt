	...
	struct options_vztt *opts_vztt)
{
	...
	struct package_list downloaded;
	/* list of vz packages, installed in VE */
	struct package_list vzpackages;
	/* list of successfully linked packages */
	struct package_list linked;
	...
	progress(PROGRESS_LINK, 0, opts_vztt->progress_fd);
	...
	ve_config_init(&vc);
	package_list_init(&installed);
	package_list_init(&downloaded);
	package_list_init(&vzpackages);
	package_list_init(&linked);
	...
	snprintf(buf, sizeof(buf), "%s/templates/vzpackages", vc.ve_private);
	if ((rc = read_nevra(buf, &vzpackages)))
		return rc;
	...
	/* include repositories for all available app templates
	   it is not bad, because of we requires exact version-release-arch */
	if ((rc = tmplset_mark(tmpl, NULL, \
			TMPLSET_MARK_OS | \
	...
	if ((rc = pm_set_root_dir(to, buf)))
		goto cleanup_2;

	/* get list of really installed packages */
	...
	for (p = installed.tqh_first; p != NULL; p = p->e.tqe_next) {
		/*
		 skip vz packages in normal mode
		 but it is one feature: if remove vz rpm on the inside VE
		 and install tne same non-vz, vzpkg will not virtualize.
		 To link all packages it's exist "slow" mode.
		! Use slow mode only.
		if (!opts_vztt->slow && find_nevra(*pkg, vzpackages))
			continue;
		*/

		/* find corresponding directory in template area */
		if (!to->pm_find_pkg_area_ex(to, p->p, buf, sizeof(buf))) {
			/* mark to download */
			to->pm_get_int_pkgname(p->p, buf, sizeof(buf));
			string_list_add(&args, buf);
			package_list_add(&downloaded, p->p);
			continue;
		}

		snprintf(cmd, sizeof(cmd), \
			"/usr/sbin/vzpkglink --quiet --skiplock "\
			"--singlepackage %s %s/%s", \
			ctid, tmpl->base->basedir, buf);
		if ((rc = exec_cmd(cmd, (opts_vztt->flags & OPT_VZTT_QUIET))))
			goto cleanup_3;
		package_list_add(&linked, p->p);
	}

	/* Download missed */
	if (!string_list_empty(&args)) {
		/* Download all packages by one transaction */
		if ((rc = to->pm_action(to, VZPKG_GET, &args)))
			goto cleanup_3;

		/* And try to relink it */
		for (p = downloaded.tqh_first; p != NULL; p = p->e.tqe_next) {
			if (!to->pm_find_pkg_area_ex(to, p->p, buf, sizeof(buf)))
				continue;
			snprintf(cmd, sizeof(cmd), \
				"/usr/sbin/vzpkglink --quiet --skiplock "\
	...
	if ((rc = merge_pkg_lists(&linked, &empty, &vzpackages)))
		goto cleanup_2;
...
cleanup_0:
	...
	package_list_clean(&installed);
	package_list_clean(&downloaded);
	package_list_clean(&vzpackages);
	package_list_clean(&linked);
	...
	progress(PROGRESS_LINK, 100, opts_vztt->progress_fd);
